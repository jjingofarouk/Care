    ...
    () => [
      {
        header: 'Patient',
        accessorFn: (row) =>
          `${row.radiologyResult.imagingOrder.patient.firstName} ${row.radiologyResult.imagingOrder.patient.lastName}`,
      },
      {
        header: 'Image URL',
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="flex gap-2">
            <IconButton
              size="small"
              className="inline-flex items-center justify-center rounded-md font-medium transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 bg-transparent text-[var(--hospital-gray-700)] border border-[var(--hospital-gray-300)] hover:bg-[var(--hospital-gray-50)] focus:ring-[var(--hospital-accent)]"
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <Button
          ...
          onClick={() => router.push('/radiology/scan-images/new')}
        >
          Create Scan Image
        </Button>
      </div>
      <DataTable
        ...
        data={scanImages}
        loading={loading}
        onRowClick={(row) => router.push(`/radiology/scan-images/${row.id}`)}
      />
    </div>
  );
}