// app/radiology/scan-images/new/page.js
'use client';

import React, { useEffect, useState } from 'react';
import { useRouter } from 'next/navigation';
import { createScanImage, getRadiologyResults } from '@/services/radiologyService';
import { ArrowLeft } from 'lucide-react';
import { Button, TextField, Select, MenuItem, InputLabel, FormControl } from '@mui/material';

export default function ScanImageNewPage() {
  ...
  useEffect(() => {
    async function fetchResults() {
      try {
        const { results } = await getRadiologyResults();
        setResults(results);
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await createScanImage(formData);
      router.push('/radiology/scan-images');
    } catch (err) {
      setError('Failed to create scan image');
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold text-[var(--hospital-gray-900)]">Create Scan Image</h1>
        <Button
          variant="outlined"
